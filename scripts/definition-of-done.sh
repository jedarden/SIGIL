...
#   - Slow: cargo test (including sigil-fuse crate)
#
...
run_slow_lane() {
  ...
  # Workspace tests
  echo "Running workspace cargo test..."
  if ! cargo test; then
    FAILURES+=("workspace cargo test failed")
    EXIT_CODE=1
  fi

  # sigil-fuse specific tests (excluded from workspace due to fuse3 dependency)
  echo "Running sigil-fuse specific tests..."
  cd crates/sigil-fuse
  if ! cargo fmt -- --check; then
    FAILURES+=("sigil-fuse fmt failed")
    EXIT_CODE=1
  fi
  if ! cargo clippy -- -D warnings; then
    FAILURES+=("sigil-fuse clippy failed")
    EXIT_CODE=1
  fi
  if ! cargo test; then
    FAILURES+=("sigil-fuse test failed")
    EXIT_CODE=1
  fi
  cd "$REPO_ROOT"
}
