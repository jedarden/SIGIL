...
2. **Nested agents**: When NEEDLE spawns multiple workers, how does each worker get its own session token? Should sigild support multi-session with per-worker scoping?
